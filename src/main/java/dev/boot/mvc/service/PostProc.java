...
public class PostProc implements PostProcInter {
  ...
  public int create(PostVO postVO) {
    ...
    String passwd_encoded = this.security.aesEncode(password);
    postVO.setPassword(passwd_encoded);

    int cnt = this.postDAOinter.create(postVO);
  ...
  public ArrayList<PostVO> list_all() {
    ArrayList<PostVO> list = this.postDAOinter.list_all();
  ...
  public ArrayList<PostVO> list_by_cateno(int cate_id) {
    ArrayList<PostVO> list = this.postDAOinter.list_by_cateno(cate_id);
  ...
  public PostVO read(int post_no) {
    PostVO postVO = this.postDAOinter.read(post_no);
  ...
  public int map(HashMap<String, Object> map) {
    int cnt = this.postDAOinter.map(map);
  ...
  public int youtube(HashMap<String, Object> map) {
    int cnt = this.postDAOinter.youtube(map);
  ...
  public ArrayList<PostVO> list_by_cateno_search(HashMap<String, Object> hashMap) {
    ArrayList<PostVO> list = this.postDAOinter.list_by_cateno_search(hashMap);
  ...
  public int list_by_cateno_search_count(HashMap<String, Object> hashMap) {
    int cnt = this.postDAOinter.list_by_cateno_search_count(hashMap);
  ...
  public ArrayList<PostVO> list_by_cateno_search_paging(HashMap<String, Object> map) {
     ...
     */
    int begin_of_page = ((int)map.get("now_page") - 1) * Posts.RECORD_PER_PAGE;
    ...
    // 2 페이지 = 10 + 10: 20
    // 3 페이지 = 20 + 10: 30
    int end_num = begin_of_page + Posts.RECORD_PER_PAGE;
    /*
    ...
    ArrayList<PostVO> list = this.postDAOinter.list_by_cateno_search_paging(map);
   ...
   */
  @Override
  public String pagingBox(int cate_id, int now_page, String word, String list_file, int search_count, int record_per_page, int page_per_block) {
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  ");
    ...
    if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성
      str.append("<span class='span_box_1'><A href='" + list_file + "?cate_id="+cate_id+"&word=" + word + "&now_page=" + _now_page
              + "'>이전</A></span>");
    }
    ...
    for (int i = start_page; i <= end_page; i++) {
      ...
      if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='" + list_file + "?cate_id="+cate_id+"&word=" + word + "&now_page=" + i + "'>" + i
                + "</A></span>");
      }
    }
  ...
  public int password_check(HashMap<String, Object> hashMap) {
    ...
    password = this.security.aesEncode(password);
    hashMap.put("password", password);

    int cnt = this.postDAOinter.password_check(hashMap);
    return cnt;
  }

  @Override
  public int update_text(PostVO postVO) {
    int cnt = this.postDAOinter.update_text(postVO);
  ...
  public int update_file(PostVO postVO) {
    int cnt = this.postDAOinter.update_file(postVO);
  ...
  public int delete(int post_no) {
    int cnt = this.postDAOinter.delete(post_no);
  ...
  public int count_by_cateno(int cate_id) {
    int cnt = this.postDAOinter.count_by_cateno(cate_id);
  ...
  public int delete_by_cateno(int cate_id) {
    int cnt = this.postDAOinter.delete_by_cateno(cate_id);
  ...
  public int increaseReplycnt(int post_no) {
    int cnt = this.postDAOinter.increaseReplycnt(post_no);
  ...
  public int decreaseReplycnt(int post_no) {
    int cnt = this.postDAOinter.decreaseReplycnt(post_no);
